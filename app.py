...
def fetch_weather(api_key, nx, ny):
    base_date, base_time = get_kst_base()
    url = "https://apis.data.go.kr/1360000/VilageFcstInfoService_2.0/getUltraSrtNcst"
    params = {"serviceKey": api_key,"pageNo": 1,"numOfRows": 100,"dataType": "JSON",
    ...
    try:
        ...
        return {"T1H": float(raw.get("T1H",20)),"REH": float(raw.get("REH",60)),
                "WSD": float(raw.get("WSD",2)),"VEC": float(raw.get("VEC",180)),
                "RN1": float(raw.get("RN1",0)),"PTY": int(raw.get("PTY",0)),
                "SKY": int(raw.get("SKY",1)),"base_date": base_date,"base_time": base_time}
...
def calc_cockroach(T, H, RN1, WSD):
    score = 0
    if 25 <= T < 30: score += 30
    ...
    if WSD < 1: score += 5
    score = min(100, score)
    if T < 15: desc = "저온으로 활동 억제. 동절기 방제 체계 유지 권장."
    elif T >= 30 and H >= 70: desc = "고온다습 — 바퀴벌레 최적 번식 조건(25–35°C, RH>70%). 즉시 점검 필요."
    elif H >= 70: desc = "습도 높음. 배수구·싱크대 하부 집중 점검 필요."
    elif T >= 25: desc = "활동 증가 온도 구간. 주 1회 트랩 확인 권장."
    else: desc = "현재 조건 양호. 정기 모니터링 유지."
...
def calc_rot(T, H):
    ...
    score = min(100, score)
    if T > 35: desc = "극고온 — Ratkowsky 모델 기준 미생물 성장 최대 구간. 냉장 보관 엄수."
    elif T >= 25 and H >= 70: desc = "고온다습. 상온 식품 노출 최소화, 냉장 온도 재확인 필요."
    elif T >= 25: desc = "부패 가속 온도. 조리 후 2시간 이내 보관 규정 준수."
    elif T < 5: desc = "저온 — 부패 억제. 냉장 체인 유지 확인."
    else: desc = "표준 관리 수준. 정상 보관 유지."
    return {"score": score, "desc": desc}

def calc_bacteria(T, H):
    if T < 5: score = 5
    ...
    elif T < 5: desc = "5°C 미만 — 대부분 세균 증식 억제."
    else: desc = "60°C 이상 — 살균 온도 도달. 고온 유지 필요."
    return {"score": score, "desc": desc}

def calc_life(T, H, WSD, PTY, RN1, SKY):
    DI = round(0.81 * T + 0.01 * H * (0.99 * T - 14.99) + 46.3)
    if T < 10:
        w_kmh = WSD * 3.6
        feels = round(13.12 + 0.6215*T - 11.37*(w_kmh**0.16) + 0.3965*(w_kmh**0.16)*T, 1)
    elif T >= 27: feels = round(T - 0.55*(1 - H/100)*(T - 14.5), 1)
    else: feels = T
...
body{background:var(--bg);color:var(--text);font-family:'Malgun Gothic','Apple SD Gothic Neo','Noto Sans KR',sans-serif;min-height:100vh}

/* ───── 위험 테두리 오버레이 ───── */
#danger-overlay{
  display:none;position:fixed;inset:0;z-index:9000;pointer-events:none;
}
#danger-overlay.active-temp{
  display:block;
  animation:overlay-temp 1s ease-in-out infinite;
}
#danger-overlay.active-hum{
  display:block;
  animation:overlay-hum 1s ease-in-out infinite;
}
#danger-overlay.active-both{
  display:block;
  animation:overlay-both .7s ease-in-out infinite;
}
@keyframes overlay-temp{
  0%,100%{box-shadow:inset 0 0 0 0 rgba(232,74,74,0)}
  50%{box-shadow:inset 0 0 0 8px rgba(232,74,74,.7),inset 0 0 80px 0 rgba(232,74,74,.18)}
}
@keyframes overlay-hum{
  0%,100%{box-shadow:inset 0 0 0 0 rgba(74,158,255,0)}
  50%{box-shadow:inset 0 0 0 8px rgba(74,158,255,.7),inset 0 0 80px 0 rgba(74,158,255,.18)}
}
@keyframes overlay-both{
  0%,100%{box-shadow:inset 0 0 0 0 rgba(232,74,74,0)}
  50%{box-shadow:inset 0 0 0 10px rgba(232,74,74,.85),inset 0 0 100px 0 rgba(232,74,74,.25)}
}

/* ───── 경보 배너 ───── */
#danger-banner{
  display:none;position:sticky;top:0;z-index:5000;
  background:#180a0a;border-bottom:2px solid var(--red);
}
#danger-banner.show{display:flex;align-items:center;justify-content:space-between;
  gap:12px;flex-wrap:wrap;padding:9px 20px}
.blink-icon{font-size:22px;animation:icon-blink .65s step-start infinite;flex-shrink:0}
@keyframes icon-blink{0%,100%{opacity:1}50%{opacity:0}}
.banner-pills{display:flex;gap:8px;flex-wrap:wrap}
.b-pill{display:flex;align-items:center;gap:5px;font-size:12px;font-weight:600;
  padding:3px 11px;border-radius:20px;
  animation:pill-fade .9s ease-in-out infinite}
@keyframes pill-fade{0%,100%{opacity:1}50%{opacity:.45}}
.b-pill.temp{background:rgba(232,74,74,.2);color:#ff8a8a;border:1px solid rgba(232,74,74,.55)}
.b-pill.hum{background:rgba(74,158,255,.2);color:#85cbff;border:1px solid rgba(74,158,255,.55)}
.banner-msg{font-size:12px;color:#ffaaaa}
.banner-time{font-size:11px;color:var(--hint);white-space:nowrap}

/* ───── 상태 표시등 행 ───── */
.ind-row{display:flex;gap:10px;flex-wrap:wrap;margin-bottom:14px}
.ind{display:flex;align-items:center;gap:8px;padding:7px 14px;border-radius:9px;
  border:1px solid var(--border);background:var(--card);font-size:13px;transition:.3s}
.ind-lamp{width:12px;height:12px;border-radius:50%;flex-shrink:0;transition:background .3s}

.ind.safe  {border-color:rgba(31,186,122,.3);background:rgba(31,186,122,.06)}
.ind.warn  {border-color:rgba(240,168,50,.4);background:rgba(240,168,50,.08)}
.ind.danger{border-color:rgba(232,74,74,.5);background:rgba(232,74,74,.1)}

.ind.safe   .ind-lamp{background:var(--green)}
.ind.warn   .ind-lamp{background:var(--amber);animation:lamp-blink .9s ease-in-out infinite}
.ind.danger .ind-lamp{background:var(--red);animation:lamp-blink .5s ease-in-out infinite}
@keyframes lamp-blink{0%,100%{opacity:1;box-shadow:none}
  50%{opacity:.2;box-shadow:0 0 8px 2px currentColor}}

.ind-label{color:var(--muted);font-size:12px}
.ind-val{font-weight:600;color:var(--text)}
.ind-st{font-size:11px;padding:2px 7px;border-radius:4px;font-weight:500}
.ind-st.safe  {background:rgba(31,186,122,.15);color:var(--green)}
.ind-st.warn  {background:rgba(240,168,50,.15);color:var(--amber)}
.ind-st.danger{background:rgba(232,74,74,.15);color:var(--red);animation:badge-blink .6s ease-in-out infinite}
@keyframes badge-blink{0%,100%{opacity:1}50%{opacity:.4}}

/* ───── 날씨 카드 위험 강조 ───── */
.w-card.flash-temp{border-color:rgba(232,74,74,.7)!important;
  animation:wcard-temp 1s ease-in-out infinite}
.w-card.flash-hum{border-color:rgba(74,158,255,.7)!important;
  animation:wcard-hum 1s ease-in-out infinite}
@keyframes wcard-temp{0%,100%{background:var(--card)}50%{background:rgba(232,74,74,.12)}}
@keyframes wcard-hum {0%,100%{background:var(--card)}50%{background:rgba(74,158,255,.12)}}

/* ───── 나머지 레이아웃 ───── */
.wrap{max-width:1200px;margin:0 auto;padding:20px 16px}
.hdr{display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;flex-wrap:wrap;gap:10px}
...
.api-bar{display:flex;gap:8px;margin-bottom:14px;flex-wrap:wrap}
...
.w-card{background:var(--card);border:1px solid var(--border);border-radius:10px;padding:12px;text-align:center;transition:.3s}
...
.update-row{display:flex;align-items:center;justify-content:space-between;font-size:11px;color:var(--hint);margin-bottom:12px}
...
<div id="danger-banner">
  ...
  <div class="banner-pills" id="banner-pills"></div>
...
<div class="wrap">
  ...
  <!-- 상태 표시등 -->
  <div class="ind-row">
    <div class="ind safe" id="ind-temp">
      <span class="ind-lamp"></span>
      <span class="ind-label">온도</span>
      <span class="ind-val" id="ind-temp-val">--°C</span>
      <span class="ind-st safe" id="ind-temp-st">대기중</span>
    </div>
    <div class="ind safe" id="ind-hum">
      <span class="ind-lamp"></span>
      <span class="ind-label">습도</span>
      <span class="ind-val" id="ind-hum-val">--%</span>
      <span class="ind-st safe" id="ind-hum-st">대기중</span>
    </div>
    <div class="ind safe" id="ind-overall">
      <span class="ind-lamp"></span>
      <span class="ind-label">종합</span>
      <span class="ind-st safe" id="ind-overall-st">위생 안전</span>
    </div>
    <div style="margin-left:auto;font-size:11px;color:var(--hint);align-self:center">
      경보 기준 — 온도 ≥ 28°C &nbsp;|&nbsp; 습도 ≥ 60%
    </div>
  </div>
  ...
  <div class="w-strip">
    <div class="w-card" id="wcard-T"><div class="w-icon">🌡️</div><div class="w-val" id="wT">--</div><div class="w-unit">°C</div><div class="w-label">기온</div></div>
    <div class="w-card" id="wcard-H"><div class="w-icon">💧</div><div class="w-val" id="wH">--</div><div class="w-unit">%</div><div class="w-label">습도</div></div>
...
let trendChart=null;
const T_THRESH=28, H_THRESH=60;

function updateDanger(T,H){
  const tDanger=T>=T_THRESH, hDanger=H>=H_THRESH, any=tDanger||hDanger, both=tDanger&&hDanger;

  const ov=document.getElementById('danger-overlay');
  ov.className='';
  if(both)       ov.classList.add('active-both');
  else if(tDanger) ov.classList.add('active-temp');
  else if(hDanger) ov.classList.add('active-hum');

  document.getElementById('wcard-T').classList.toggle('flash-temp',tDanger);
  document.getElementById('wcard-H').classList.toggle('flash-hum',hDanger);

  const itEl=document.getElementById('ind-temp');
  document.getElementById('ind-temp-val').textContent=T+'°C';
  const itSt=document.getElementById('ind-temp-st');
  if(tDanger){itEl.className='ind danger';itSt.className='ind-st danger';itSt.textContent='위험 ≥'+T_THRESH+'°C'}
  else if(T>=25){itEl.className='ind warn';itSt.className='ind-st warn';itSt.textContent='주의'}
  else{itEl.className='ind safe';itSt.className='ind-st safe';itSt.textContent='정상'}

  const ihEl=document.getElementById('ind-hum');
  document.getElementById('ind-hum-val').textContent=H+'%';
  const ihSt=document.getElementById('ind-hum-st');
  if(hDanger){ihEl.className='ind danger';ihSt.className='ind-st danger';ihSt.textContent='위험 ≥'+H_THRESH+'%'}
  else if(H>=50){ihEl.className='ind warn';ihSt.className='ind-st warn';ihSt.textContent='주의'}
  else{ihEl.className='ind safe';ihSt.className='ind-st safe';ihSt.textContent='정상'}

  const ioEl=document.getElementById('ind-overall');
  const ioSt=document.getElementById('ind-overall-st');
  if(both){ioEl.className='ind danger';ioSt.className='ind-st danger';ioSt.textContent='🚨 고온·고습 복합위험'}
  else if(tDanger){ioEl.className='ind danger';ioSt.className='ind-st danger';ioSt.textContent='🌡️ 고온 경보'}
  else if(hDanger){ioEl.className='ind danger';ioSt.className='ind-st danger';ioSt.textContent='💧 고습 경보'}
  else{ioEl.className='ind safe';ioSt.className='ind-st safe';ioSt.textContent='위생 안전'}

  const banner=document.getElementById('danger-banner');
  if(any){
    banner.classList.add('show');
    let pills='';
    if(tDanger) pills+=`<span class="b-pill temp">🌡️ ${T}°C — ${T_THRESH}°C 초과</span>`;
    if(hDanger) pills+=`<span class="b-pill hum">💧 ${H}% — ${H_THRESH}% 초과</span>`;
    document.getElementById('banner-pills').innerHTML=pills;
    document.getElementById('banner-msg').textContent=
      both?'고온·고습 복합 위험 — 즉시 환기·냉방·점검 필요':
      tDanger?'온도 임계값 초과 — 식품 냉장 보관 및 주방 점검 바랍니다':
              '습도 임계값 초과 — 환기·제습 장치 가동 권장';
    document.getElementById('banner-time').textContent=
      '경보 발생 '+new Date().toLocaleTimeString('ko-KR');
  } else {
...
async function loadData(){
  ...
  try{const res=await fetch(url);const d=await res.json();
    if(d.error){alert('API 오류: '+d.error);return}renderAll(d);
  }catch(e){alert('서버 오류: '+e.message)}
}

function scoreLevel(s){
  if(s<25)return['안전','lv-safe'];if(s<50)return['보통','lv-safe'];
  if(s<70)return['주의','lv-caution'];if(s<85)return['경보','lv-caution'];
  return['위험','lv-danger'];
}
...
function renderTrend(currentT){
  const h=new Date().getHours();const labels=[],vals=[];
  for(let i=23;i>=0;i--){
    const hh=((h-i)+24)%24;
...
function renderAlerts(alerts){
  ...
  alerts.forEach(a=>{box.innerHTML+=`<div class="alert-item ${cls[a.type]}"><span class="a-icon">${icon[a.type]}</span><span>${a.msg}</span></div>`});
}
...
def api_weather():
    ...
    T,H,WSD,RN1,PTY,SKY = weather["T1H"],weather["REH"],weather["WSD"],weather["RN1"],weather["PTY"],weather["SKY"]
    return jsonify({
        "weather": weather,
